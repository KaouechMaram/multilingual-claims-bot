...
- **Multi-issue detection** — a single message such as "my card is blocked
...
- **Darija-aware prompting**: informal Latin-script Tunisian Arabic
  examples are built into the classification prompt for better recognition
...
- Text and voice chat (microphone input transcribed via Whisper; live
  captions shown during recording via the browser's speech recognition,
  while the message actually sent uses the more reliable Whisper
  transcription).
...
- Thumbs-up / thumbs-down feedback on each bot reply.
- Listen to replies (FR/AR text-to-speech).

## Supervisor features (admin role)
...
- Live human-assistance escalation queue (Server-Sent Events, no manual
  refresh), with a "Handle" action to mark a case as taken.
- Ticket status changes, which automatically trigger a notification (email
  — see note below).
- Demo supervisor account: `admin` / `Admin1234!`

Note: email/SMS notifications are a documented stub, not a real send.
`notificationService.js` logs the action (console output and a
`notifications_log` table) instead of actually emailing or texting, since
no SMTP/Twilio account is configured in this environment. The interface is
ready for a real integration (nodemailer, SendGrid, Twilio) — see the
comments in the file for the exact integration point.
...
(login and chat), account lockout after repeated failures, a shared
internal key protecting `chatbot-api` even if accidentally exposed,
role-based access control on supervisor routes, per-user file isolation,
centralized error handling (no stack traces leaked to the client), a
startup guard that refuses to run in production with default secrets, and
an audit log.
...
no native compilation needed, unlike packages such as `better-sqlite3`).
...
another browser or private window while a conversation is ongoing with
`demo`.
...
| Auth and API gateway | Node.js, Express, JWT (httpOnly cookie), bcrypt, SQLite |
| Chatbot microservice | Python, FastAPI, Groq API (LLM), Whisper (speech-to-text), gTTS (text-to-speech) |
| Data and modeling background | pandas, scikit-learn, google-play-scraper |

---

## Project background — from raw data to product

This chatbot was not built on a toy dataset. A summary of the earlier data
...
- **Taxonomy**: 11 main categories and 49 sub-categories defined with the
  internship supervisor, covering the full range of monetics complaints.
- **First classification pass**: LLM-based labeling (Groq — Llama 3, later
  combined with Google Gemini to work around free-tier rate limits).
  Manual audit on a roughly 200-row sample measured 39% precision; errors
  were diagnosed, a missing category added, and the prompt strengthened
  with exclusion rules and targeted examples.
...
- **Key methodological finding**: a naive random train/test split leaked
  near-identical phrasings between sets, inflating a local TF-IDF plus
